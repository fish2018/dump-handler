...
func alarm() {
	//key=项目组，value=钉钉token
	tokenMap := map[string]string{
		"ops":"xxx",
	}
	dingdingToken = tokenMap[projectGroup]
	ossurl := fmt.Sprintf("http://%s.oss-cn-shanghai-internal.aliyuncs.com/%s/k8s/jvm/%s/%s-%s", bucketName,env,folder,podId,postfix) //建议OSS内网，"oss-cn-shanghai-internal"更换成自己的endpoint
